...
try:
    ...
    if "_save_state" in content and "_load_state" in content:
        if "state_file" in content and "json.dump" in content:
            ...
            print("[PARTIAL]: State methods exist but persistence unclear")
...
try:
    ...
    if "enforce_https" in content or "request.is_secure" in content:
        if "redirect" in content and "https://" in content:
            ...
            print("[PARTIAL]: HTTPS check exists but redirect unclear")
...
try:
    ...
    if "lockout_state.json" in content:
        ...
        print("[WARNING]: lockout_state.json not in .gitignore")
...
if issues_found:
    ...
    print("\n[PASS] ALL CRITICAL AND HIGH PRIORITY FIXES VERIFIED")
    sys.exit(0)
