...
b = {
    'cash': 1000, 'ath': 200, 'athm': 50, 'visa': 300,
    'mc': 150, 'amex': 75, 'disc': 25, 'wic': 100,
    'mcs': 80, 'sss': 20,
    'float': 150, 'actual': 1000, 'payouts': 0
}
...
try:
    ...
    def _check_math(data: dict):
        ...
        if abs(gross - exp_gross) > TOL:
            return False, f"Gross mismatch"
        if abs(net - exp_net) > TOL:
            return False, f"Net mismatch"
        if abs(variance - exp_variance) > TOL:
            return False, f"Variance mismatch"
        return True, ""

    # 4.1 Valid payload passes
    valid = {
        'gross': 700, 'net': 625, 'variance': 0,
    ...
    tampered_var = {
        'gross': 700, 'net': 625, 'variance': 999,
        'breakdown': {'cash': 500, 'visa': 200, 'payouts': 75, 'float': 150, 'actual': 575}
    }
