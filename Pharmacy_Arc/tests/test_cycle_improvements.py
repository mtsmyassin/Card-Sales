...
class TestUXFixes:
    ...
    def test_password_field_is_type_password(self, ui):
        ...
        assert 'type="password"' in snippet, f"Password field must be type=password. Got: {snippet!r}"

    def test_password_field_not_type_text(self, ui):
        """Regression: password field was previously type=text — must never return."""
        idx = ui.find('id="u_pass"')
        snippet = ui[max(0, idx - 60) : idx + 100]
        assert 'type="text"' not in snippet, "Password field reverted to type=text — plaintext passwords visible!"
...
class TestDiagnosticsAdminClientField:
    ...
    def test_admin_client_key_present_in_source(self):
        src = _app_source()
        assert '"admin_client"' in src, "diagnostics response dict must include 'admin_client' key"

    def test_admin_client_configured_message_present(self):
        src = _app_source()
        assert '"configured"' in src, "diagnostics must report 'configured' when supabase_admin is set"

    def test_admin_client_not_set_warning_present(self):
        src = _app_source()
        assert "NOT SET" in src, "diagnostics must warn 'NOT SET' when SUPABASE_SERVICE_KEY is missing"
    ...
    def test_storage_diagnostics_use_admin_client(self):
        ...
        assert "_storage_client" in src, "Storage diagnostics must use _storage_client (supabase_admin or supabase)"
...
class TestBotInsertErrorHandling:
    ...
    def test_raises_on_db_failure_does_not_queue(self):
        ...
        failing_admin.table.return_value.insert.return_value.execute.side_effect = RuntimeError(
            "DB insert rejected by RLS"
        )

        with patch("extensions.supabase_admin", failing_admin), patch("extensions.supabase", None):
    ...
    def test_prefers_admin_client_over_anon(self):
        ...
        mock_admin.table.return_value.insert.return_value.execute.return_value.data = [{"id": 99}]
        mock_anon = MagicMock()

        with patch("extensions.supabase_admin", mock_admin), patch("extensions.supabase", mock_anon):
            from telegram_bot import save_audit_entry
    ...
    def test_falls_back_to_anon_when_admin_none(self):
        ...
        mock_anon = MagicMock()
        mock_anon.table.return_value.insert.return_value.execute.return_value.data = [{"id": 55}]

        with patch("extensions.supabase_admin", None), patch("extensions.supabase", mock_anon):
            from telegram_bot import save_audit_entry
