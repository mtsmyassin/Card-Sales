...
class TestSessionTimeout:
    ...
    def test_active_session_refreshes_last_active(self):
        ...
        _ = client.get("/api/list")  # side-effect: triggers before_request
    ...
    def test_expired_session_cleared(self):
        ...
        assert resp.status_code == 401, "Expired session must return 401 (session cleared, require_auth fires)"
    ...
    def test_login_stamps_last_active(self):
        ...
        assert "last_active" in login_block, "login() must stamp session['last_active'] on successful authentication"
    ...
    def test_session_timeout_uses_timezone_aware_utc(self):
        ...
        assert "timezone.utc" in hook_block or "UTC" in hook_block, (
            "enforce_session_timeout must use timezone-aware UTC (datetime.now(UTC) or datetime.now(timezone.utc))"
        )
        assert "utcnow()" not in hook_block, "enforce_session_timeout must not use deprecated datetime.utcnow()"
...
class TestPayloadSizeLimit:
    def test_max_content_length_set(self):
        ...
        assert limit == 5 * 1024 * 1024, f"MAX_CONTENT_LENGTH must be 5MB (5242880), got {limit}"

    def test_413_handler_returns_json(self):
        """413 error handler must return JSON with an error message."""
        _load_app()  # ensure app loads without error
        src = open(
...
class TestAuditLogEncoding:
    def test_file_opens_use_utf8(self):
        ...
        for o in opens:
            # Binary mode opens (rb/wb) don't need encoding
            if '"rb"' in o or "'rb'" in o or '"wb"' in o or "'wb'" in o:
                continue
            assert "utf-8" in o or "encoding" in o, f"open() call missing encoding='utf-8': {o}"
    ...
    def test_supabase_write_before_file_write(self):
        ...
        assert db_pos < file_pos, "AuditLogger.log() must call _write_to_db() before writing to the local file"
...
class TestOfflineQueueCap:
    def test_save_to_queue_returns_bool(self):
        ...
        app_module = _load_app()
        # save_to_queue may live in helpers.offline_queue, not directly on app module
        save_fn = getattr(app_module, "save_to_queue", None)
        if save_fn is None:
            from helpers.offline_queue import save_to_queue as save_fn
        assert callable(save_fn), "save_to_queue must exist"
    ...
    def test_queue_max_size_constant_defined(self):
        ...
        assert "OFFLINE_QUEUE_MAX_SIZE" in src, "OFFLINE_QUEUE_MAX_SIZE constant must be defined"
...
class TestSupabaseStartupRetry:
    def test_init_supabase_helper_exists(self):
        ...
        assert "def _init_supabase(" in src, "app.py must define _init_supabase() retry helper"
