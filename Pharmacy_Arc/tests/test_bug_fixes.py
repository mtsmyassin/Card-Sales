...
class TestSyncUsesAdminClient:
    ...
    def test_sync_dup_check_uses_admin(self):
        ...
        assert "_db = extensions.get_db()" in block, "sync() must assign _db = extensions.get_db() before dup check"

    def test_sync_insert_uses_db_variable(self):
        block = self._get_sync_block()
        assert '.table("audits").insert' in block, "sync() must use admin client for INSERT"
...
class TestOfflineQueueEncoding:
    def test_utf8_roundtrip(self, tmp_path, monkeypatch):
        ...
        assert len(result) == 1
        assert result[0]["store"] == "Farmacía #1", "Accented characters must survive UTF-8 queue roundtrip"
...
class TestSyncLogsExceptions:
    def test_bare_except_replaced(self):
        ...
        assert "except:" not in sync_block, "sync() must not use bare except: — use except Exception as exc"
    ...
    def test_sync_exception_logged(self):
        ...
        sync_block = src[src.index("def sync():") : src.index("def update():")]
        assert "logger.warning" in sync_block, "sync() must log a warning when an insert fails"
...
class TestAPSchedulerShutdown:
    def test_atexit_registered(self):
        ...
        assert "atexit.register" in src, "APScheduler must register atexit shutdown to prevent thread leak on deploy"
...
class TestCSRFProtection:
    ...
    def test_telegram_webhook_is_csrf_exempt(self):
        ...
        wh_block = src[src.index('@bp.route("/api/telegram/webhook"') : src.index("def telegram_webhook():") + 50]
