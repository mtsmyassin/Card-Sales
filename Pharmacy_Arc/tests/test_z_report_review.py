...
class TestRecalculate:
    ...
    def test_net_equals_gross_minus_payouts(self):
        bd = {"cash": 200, "ath": 0, "athm": 0, "visa": 0, "mc": 0, "amex": 0, "disc": 0, "wic": 0, "mcs": 0, "sss": 0}
    ...
    def test_payouts_exceed_gross_raises(self):
        ...
        bd = {"cash": 10, "ath": 0, "athm": 0, "visa": 0, "mc": 0, "amex": 0, "disc": 0, "wic": 0, "mcs": 0, "sss": 0}
    ...
    def test_zero_all_fields(self):
        bd = {"cash": 0, "ath": 0, "athm": 0, "visa": 0, "mc": 0, "amex": 0, "disc": 0, "wic": 0, "mcs": 0, "sss": 0}
        r = self._calc(bd)
...
def flask_app():
    ...
    with (
        ...
        _app.app.config["SECRET_KEY"] = "test-secret"  # noqa: S105 — test fixture
...
class TestZReportList:
    ...
    def test_manager_can_list(self, client, flask_app, manager_session):
        ...
        flask_app.supabase_admin.table.return_value.select.return_value.is_.return_value.order.return_value.eq.return_value.range.return_value.execute.return_value = mock_result
        r = client.get("/api/z-reports")
    ...
    def test_status_filter_applied(self, client, flask_app, manager_session):
        ...
        chain = flask_app.supabase_admin.table.return_value.select.return_value.is_.return_value.order.return_value
...
class TestZReportLock:
    ...
    def _setup_mock_audit(self, flask_app, audit_dict):
        ...
        flask_app.supabase_admin.table.return_value.select.return_value.eq.return_value.is_.return_value.single.return_value.execute.return_value = mock
    ...
    def test_lock_pending_review(self, client, flask_app, manager_session):
        ...
        flask_app.supabase_admin.table.return_value.insert.return_value.execute.return_value = MagicMock()
    ...
    def test_lock_held_by_other_user_unexpired(self, client, flask_app, manager_session):
        ...
        self._setup_mock_audit(flask_app, self._audit(status="IN_REVIEW", locked_by="othermanager", locked_at=recent))
        r = client.post("/api/z-reports/42/lock")
        assert r.status_code == 409
    ...
    def test_lock_held_by_other_expired_succeeds(self, client, flask_app, manager_session):
        ...
        self._setup_mock_audit(flask_app, self._audit(status="IN_REVIEW", locked_by="othermanager", locked_at=old))
        self._setup_update_chain(flask_app)
        flask_app.supabase_admin.table.return_value.insert.return_value.execute.return_value = MagicMock()
        r = client.post("/api/z-reports/42/lock")
        assert r.status_code == 200
    ...
    def test_re_lock_own_audit_allowed(self, client, flask_app, manager_session):
        ...
        recent = (datetime.now(UTC) - timedelta(minutes=5)).isoformat()
        self._setup_mock_audit(flask_app, self._audit(status="IN_REVIEW", locked_by="testmanager", locked_at=recent))
        self._setup_update_chain(flask_app)
        flask_app.supabase_admin.table.return_value.insert.return_value.execute.return_value = MagicMock()
        r = client.post("/api/z-reports/42/lock")
        assert r.status_code == 200
...
class TestZReportApprove:
    def _setup(self, flask_app, status="IN_REVIEW", locked_by="testmanager"):
        ...
        flask_app.supabase_admin.table.return_value.select.return_value.eq.return_value.is_.return_value.single.return_value.execute.return_value = audit_mock
        ...
        existing_mock.data = []
        flask_app.supabase_admin.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = existing_mock
        flask_app.supabase_admin.table.return_value.insert.return_value.execute.return_value = MagicMock()

    def test_approve_success(self, client, flask_app, manager_session):
        self._setup(flask_app)
        r = client.post(
            "/api/z-reports/1/approve",
    ...
    def test_approve_not_in_review(self, client, flask_app, manager_session):
        ...
        r = client.post("/api/z-reports/1/approve", data=json.dumps({}), content_type="application/json")
        assert r.status_code == 409
...
class TestZReportRejectAmend:
    def _setup_audit(self, flask_app, status, locked_by="testmanager"):
        ...
        flask_app.supabase_admin.table.return_value.select.return_value.eq.return_value.is_.return_value.single.return_value.execute.return_value = m
        # Self-referencing update chain for guarded updates
        update_chain = flask_app.supabase_admin.table.return_value.update.return_value
        update_chain.eq.return_value = update_chain
        update_chain.is_.return_value = update_chain
        ...
        existing = MagicMock()
        existing.data = []
        flask_app.supabase_admin.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = existing
        flask_app.supabase_admin.table.return_value.insert.return_value.execute.return_value = MagicMock()

    def test_reject_requires_reason(self, client, flask_app, manager_session):
        self._setup_audit(flask_app, "IN_REVIEW")
        r = client.post("/api/z-reports/1/reject", data=json.dumps({}), content_type="application/json")
        assert r.status_code == 400
