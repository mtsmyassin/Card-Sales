...
class TestUploadImageToStorage:
    def test_raises_when_supabase_admin_none(self):
        ...
        with patch("extensions.supabase_admin", None):
            with pytest.raises(Exception):  # noqa: B017 — testing that any error is raised when admin is None
...
class TestSaveAuditEntryReturnsId:
    def test_returns_entry_id_on_success(self):
        ...
        with patch("extensions.supabase_admin", None), patch("extensions.supabase", mock_supabase):
...
def make_text_update(telegram_id: int, text: str) -> dict:
    return {"message": {"from": {"id": telegram_id, "username": "testuser"}, "chat": {"id": telegram_id}, "text": text}}
...
class TestNewBotFlow:
    def test_photo_leads_to_awaiting_date(self):
        ...
        bot_state.clear()
        bot_state[100] = {"state": "REGISTERED", "store": "Carimas #1", "username": "manager", "retry_count": 0}
        replies = []
    ...
    def test_date_ok_leads_to_awaiting_register(self):
        ...
        bot_state[101] = {
            ...
            "store": "Carimas #1",
            "username": "manager",
            "pending_data": GOOD_OCR.copy(),
            "pending_image_bytes": b"bytes",
            "retry_count": 0,
        }
        replies = []
        with patch("telegram_bot.send_message", side_effect=lambda c, t, **kw: replies.append(t)):
