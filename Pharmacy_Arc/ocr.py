...
class OCRParseError(Exception):
    """Raised when Gemini's response cannot be parsed as valid JSON."""


def extract_z_report(image_bytes: bytes) -> dict:
    """
    Send receipt image to Gemini Vision API and return extracted fields as dict.
    ...
    try:
        ...
        raise OCRParseError(f"Gemini returned non-JSON: {raw!r}") from e
