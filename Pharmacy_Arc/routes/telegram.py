...
from helpers.auth_utils import can_access_photo, is_admin_role, require_auth
...
def telegram_webhook():
    ...
    if not hmac.compare_digest(incoming_secret, expected_secret):
        logger.warning(
            "Telegram webhook: invalid secret token from %s",
...
def get_zreport_image(audit_id: int):
    ...
    try:
        ...
        signed = (
            extensions.get_db()
            .storage.from_(Config.STORAGE_BUCKET)
            .create_signed_url(image_path, Config.STORAGE_URL_EXPIRY_SECONDS)
        )
...
def get_photo_signed_url():
    ...
    try:
        ...
        logger.error(f"[signed_url] FAILED photo_id={photo_id}: {e}", exc_info=True)
...
def delete_photo(photo_id):
    ...
    try:
        _db = extensions.get_db()
        photo_data = row0(_db.table("z_report_photos").select("*").eq("id", photo_id).maybe_single().execute())
        ...
        logger.info(f"[delete_photo] photo_id={photo_id} deleted by {session.get('user')!r}")
