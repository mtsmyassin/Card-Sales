...
from helpers.supabase_types import row0, rows
...
def _zr_validate_breakdown(payouts_total: float, payouts_breakdown: dict | None) -> None:
    ...
    if abs(total - payouts_total) > 0.01:
        raise ValueError(f"payouts_breakdown sum {total:.2f} != payouts_total {payouts_total:.2f}")
...
def _zr_compensate(db, audit_id: int, old_status: str, old_locked_by, old_locked_at, completed_steps: list) -> None:
    ...
    for step_name, step_id in reversed(completed_steps):
        try:
            if step_name == "update_audit_status":
                ...
                if latest_rows:
                    db.table("z_report_reviews").update({"is_current": True}).eq("id", latest_rows[0]["id"]).execute()
...
def _zr_guarded_update(
    db, audit_id: int, update_data: dict, expected_status: str, expected_locked_by=_SENTINEL
) -> bool:
...
def zr_list():
    ...
    try:
        ...
        q = (
            db.table("audits")
            .select("id,store,date,review_status,review_locked_by,review_locked_at")
            .is_("deleted_at", "null")
...
def zr_detail(audit_id: int):
    ...
    try:
        audit = row0(
            db.table("audits").select("*").eq("id", audit_id).is_("deleted_at", "null").maybe_single().execute()
        )
        if not audit:
...
def zr_lock(audit_id: int):
    ...
    try:
        audit = row0(
            db.table("audits")
            .select("id,store,review_status,review_locked_by,review_locked_at")
            .eq("id", audit_id)
            .is_("deleted_at", "null")
            .single()
            .execute()
        ...
        if status in ("PENDING_REVIEW", "NEEDS_ASSIGNMENT"):
            ...
            if locked_by == actor:
                ...
                lock_time = datetime.fromisoformat(locked_at.replace("Z", "+00:00")) if locked_at else None
                if lock_time and datetime.now(UTC) - lock_time < timedelta(minutes=Config.ZREPORT_LOCK_TIMEOUT_MINUTES):
                    return jsonify(
        ...
        update_data = {
            ...
            "review_locked_at": datetime.now(UTC).isoformat(),
        }

        # Try guarded update against the current status
        updated = _zr_guarded_update(db, audit_id, update_data, old_status, expected_locked_by=locked_by)
        if not updated:
...
def zr_unlock(audit_id: int):
    ...
    try:
        audit = row0(
            db.table("audits")
            .select("id,store,review_status,review_locked_by")
            .eq("id", audit_id)
            .is_("deleted_at", "null")
            .single()
            .execute()
        ...
        updated = _zr_guarded_update(db, audit_id, update_data, "IN_REVIEW", expected_locked_by=expected_locker)
        if not updated:
            return jsonify(error="Audit state changed — please refresh and retry", code="CONFLICT"), 409
...
def zr_approve(audit_id: int):
    ...
    try:
        ...
        return jsonify(error="Payouts breakdown validation failed", code="INVALID_INPUT"), 400
    ...
    try:
        ...
        try:
            ...
            return jsonify(error="Z-report recalculation failed due to invalid data", code="INVALID_INPUT"), 400
        ...
        try:
            ...
            db.table("z_report_reviews").update({"is_current": False}).eq("audit_id", audit_id).eq(
                "is_current", True
            ).execute()
            ...
            _zr_compensate(
                db, audit_id, old_status, audit.get("review_locked_by"), audit.get("review_locked_at"), completed_steps
            )
            return jsonify(error="Internal server error", code="INTERNAL_ERROR"), 500
...
def zr_amend(audit_id: int):
    ...
    try:
        ...
        if audit["review_status"] != "FINAL_APPROVED":
            return jsonify(
                error=f"Can only amend FINAL_APPROVED audits (status: {audit['review_status']})", code="CONFLICT"
            ), 409
...
def zr_history(audit_id: int):
    ...
    try:
        ...
        audit_data = row0(
            db.table("audits").select("id,store").eq("id", audit_id).is_("deleted_at", "null").maybe_single().execute()
        )
        if not audit_data:
            return jsonify(error="Audit not found", code="NOT_FOUND"), 404
...
def zr_audit_log(audit_id: int):
    ...
    if db is None:
        return jsonify(error="Z-report review service unavailable", code="SERVICE_UNAVAILABLE"), 503
    try:
        result = db.table("z_report_audit_log").select("*").eq("audit_id", audit_id).order("ts", desc=True).execute()
        return jsonify(rows(result))
    except Exception as e:
...
def zr_unlock_timed_out():
    ...
    actor = session["user"]
    ip = request.remote_addr
    cutoff = (datetime.now(UTC) - timedelta(minutes=Config.ZREPORT_LOCK_TIMEOUT_MINUTES)).isoformat()
    try:
        stale_rows = rows(
            db.table("audits")
            .select("id,review_locked_by")
            .eq("review_status", "IN_REVIEW")
            .is_("deleted_at", "null")
            .lt("review_locked_at", cutoff)
            .execute()
        )
        count = 0
        for row in stale_rows:
            updated = _zr_guarded_update(
                ...
                expected_status="IN_REVIEW",
                expected_locked_by=row["review_locked_by"],
            )
            if updated:
                _zr_log(
                    db,
